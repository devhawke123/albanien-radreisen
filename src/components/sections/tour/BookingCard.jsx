...
export default function BookingCard() {
  ...
  const [requestForm, setRequestForm] = useState({ name: "", email: "", phone: "", address: "", message: "" });
  ...
  const dateGuestAddonFields = (showAddonQuantities = true) => (
    <>
      ...
      <label className="block">
        ...
        <input
          type="text"
          readOnly
          value={formatDisplayDate(selectedDeparture.checkIn, locale)}
      ...
      <label className="block">
        ...
        <input
          type="text"
          readOnly
          value={formatDisplayDate(selectedDeparture.checkOut, locale)}
          className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 font-sans text-sm text-gray-800"
        />
      </label>
      ...
      <div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-gray-200 px-3 py-2">
          <span className="font-sans text-sm text-gray-700">
            {t("tourPage.booking.adult")} <strong>{formatEuro(tour.basePrice)}</strong>
          </span>
          <QuantityStepper
            value={guests}
            min={1}
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const selectedAddonLines = selectedAddons.map((id) => {
        const addon = addons.find((item) => item.id === id);
        const qty = formTab === "booking" ? (addonQuantities[id] ?? 1) : 1;
      ...
      await submitRequest({
        ...
        guests,
        addonsSummary: selectedAddonLines.length
          ? selectedAddonLines.join("; ")
          : t("tourPage.booking.noneSelected"),
        locale,
      });

      setStatus("success");
      setRequestForm({ name: "", email: "", phone: "", address: "", message: "" });
    } catch (error) {
      setStatus("error");
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <span className="mt-0 text-brand" aria-hidden>
          📍
        </span>
        <div>
      ...
      <div className="relative my-5 flex items-center justify-center">
        <div className="absolute inset-x-0 top-1/2 border-t border-dashed border-brand" aria-hidden />
        <span className="relative bg-white px-1 text-xl text-brand" aria-hidden>
          ✈
        </span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" key={formTab}>
        {formTab === "booking" ? (
          dateGuestAddonFields(true)
        ) : (
          <>
            <label className="block">
              <span className="mb-1.5 block font-sans text-sm text-gray-600">
                {t("tourPage.booking.fullName")} <span className="text-brand">*</span>
              </span>
              <input
                type="text"
                required
                autoComplete="name"
                placeholder={t("tourPage.booking.namePlaceholder")}
            ...
            <label className="block">
              ...
              <input
                ...
                required
                autoComplete="email"
                placeholder={t("tourPage.booking.emailPlaceholder")}
            ...
            <label className="block">
              <span className="mb-1.5 block font-sans text-sm text-gray-600">
                {t("tourPage.booking.phone")} <span className="text-brand">*</span>
              </span>
              <input
                type="tel"
                required
                autoComplete="tel"
                placeholder={t("tourPage.booking.phonePlaceholder")}
            ...
            <label className="block">
              <span className="mb-1.5 block font-sans text-sm text-gray-600">
                {t("tourPage.booking.address")} <span className="text-brand">*</span>
              </span>
              <input
                type="text"
                required
                autoComplete="street-address"
                placeholder={t("tourPage.booking.addressPlaceholder")}
                value={requestForm.address}
                onChange={(event) =>
                  setRequestForm((current) => ({ ...current, address: event.target.value }))
                }
                className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-3 font-sans text-sm"
              />
            </label>

            {dateGuestAddonFields(false)}
            ...
            <label className="block">
              ...
              <textarea
                rows={4}
                placeholder={t("tourPage.booking.messagePlaceholder")}
